...
# ============================================================
{
  ...
  wifiResetScript = pkgs.writeShellScriptBin "wifi-reset" ''
    ...
    if [[ "''${1:-}" == "-h" || "''${1:-}" == "--help" ]]; then
      ...
      echo "Resets Wi-Fi configuration by stopping dependent services, removing the setup marker, and restarting wifi-setup."
      exit 0
    fi
  ...
  wifiSetupScript = pkgs.writeShellScriptBin "wifi-setup" ''
    ...
    if [[ "''${1:-}" == "-h" || "''${1:-}" == "--help" ]]; then
      echo "Usage: $(basename "$0")"
      echo "Starts a captive portal for Wi-Fi setup on first boot. Creates a marker file on success to prevent re-running."
      exit 0
    fi
